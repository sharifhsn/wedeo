// VP9 data tables.
//
// Translated from FFmpeg's libavcodec/vp9data.c and vp9data.h.
// All tables are `const` to allow compile-time embedding.

// ---------------------------------------------------------------------------
// Block width/height table  (ff_vp9_bwh_tab)
// ---------------------------------------------------------------------------

/// Block width and height in 4×4 units, indexed by [use_uv][block_size][w/h].
///
/// Index 0 gives luma dimensions; index 1 gives chroma dimensions
...
/// Partition tree: maps bit decisions to PARTITION_* values.
/// Negative entries are leaf values (negated).
pub const PARTITION_TREE: [[i8; 2]; 3] = [
    [-(0i8), 1],      // '0' → PARTITION_NONE
    [-(1i8), 2],      // '10' → PARTITION_H
    [-(2i8), -(3i8)], // '110' → PARTITION_V, '111' → PARTITION_SPLIT
];
...
pub const INTRAMODE_TREE: [[i8; 2]; 9] = [
    ...
    [4, 6],
    [-(1i8), 5],      // HOR_PRED = 1
    [-(4i8), -(5i8)], // DIAG_DOWN_RIGHT_PRED = 4, VERT_RIGHT_PRED = 5
    [-(3i8), 7],      // DIAG_DOWN_LEFT_PRED = 3
    [-(7i8), 8],      // VERT_LEFT_PRED = 7
    [-(6i8), -(8i8)], // HOR_DOWN_PRED = 6, HOR_UP_PRED = 8
];
...
pub const INTER_MODE_TREE: [[i8; 2]; 3] = [
    [-(12i8), 1],       // ZEROMV = 12
    [-(10i8), 2],       // NEARESTMV = 10
    [-(11i8), -(13i8)], // NEARMV = 11, NEWMV = 13
];

/// Filter tree.
pub const FILTER_TREE: [[i8; 2]; 2] = [[-(0i8), 1], [-(1i8), -(2i8)]];

/// MV joint tree.
pub const MV_JOINT_TREE: [[i8; 2]; 3] = [
    [-(0i8), 1],      // MV_JOINT_ZERO = 0
    [-(1i8), 2],      // MV_JOINT_H = 1
    [-(2i8), -(3i8)], // MV_JOINT_V = 2, MV_JOINT_HV = 3
];
...
/// MV fractional-pixel tree.
pub const MV_FP_TREE: [[i8; 2]; 3] = [[-(0i8), 1], [-(1i8), 2], [-(2i8), -(3i8)]];

// ---------------------------------------------------------------------------
// Filter LUT
// ---------------------------------------------------------------------------

/// Maps filter tree output (0-2) to FilterMode index.
/// Index 0 → FILTER_8TAP_REGULAR (1), 1 → FILTER_8TAP_SMOOTH (0),
/// 2 → FILTER_8TAP_SHARP (2).
pub const FILTER_LUT: [u8; 3] = [1, 0, 2];

// ---------------------------------------------------------------------------
// Keyframe probability tables
// ---------------------------------------------------------------------------

/// Default keyframe partition probabilities [block_level][above+left_ctx][3].
...
/// Default keyframe Y mode probabilities [above_mode][left_mode][9].
...
/// Default keyframe UV mode probabilities [y_mode][9].
...
// ---------------------------------------------------------------------------
// Quantizer lookup tables  (ff_vp9_dc_qlookup / ff_vp9_ac_qlookup)
// ---------------------------------------------------------------------------

/// DC quantizer lookup table indexed by [bpp_index][qindex].
/// bpp_index: 0 = 8-bit, 1 = 10-bit, 2 = 12-bit.
pub const DC_QLOOKUP: [[i16; 256]; 3] = [
    /* 8-bit */
    [
...
/// AC quantizer lookup table indexed by [bpp_index][qindex].
pub const AC_QLOOKUP: [[i16; 256]; 3] = [
    /* 8-bit */
    [
...
// ---------------------------------------------------------------------------
// Intra transform type lookup  (ff_vp9_intra_txfm_type)
// ---------------------------------------------------------------------------

/// Maps intra prediction mode index to TxfmType (0=DCT_DCT, 1=DCT_ADST,
/// 2=ADST_DCT, 3=ADST_ADST).  Indexed 0..=13 (VERT_PRED..NEWMV).
pub const INTRA_TXFM_TYPE: [u8; 14] = [
    2, // VERT_PRED     → ADST_DCT
    1, // HOR_PRED      → DCT_ADST
    0, // DC_PRED       → DCT_DCT
    0, // DIAG_DOWN_LEFT_PRED → DCT_DCT
    3, // DIAG_DOWN_RIGHT_PRED → ADST_ADST
    2, // VERT_RIGHT_PRED → ADST_DCT
    1, // HOR_DOWN_PRED  → DCT_ADST
    2, // VERT_LEFT_PRED → ADST_DCT
    1, // HOR_UP_PRED    → DCT_ADST
    3, // TM_VP8_PRED    → ADST_ADST
    0, // NEARESTMV (10) → DCT_DCT
    0, // NEARMV    (11) → DCT_DCT
    0, // ZEROMV    (12) → DCT_DCT
    0, // NEWMV     (13) → DCT_DCT
];
...
/// Default (diagonal) 4×4 scan order.
pub const DEFAULT_SCAN_4X4: [i16; 16] = [0, 1, 4, 5, 2, 8, 3, 6, 12, 9, 7, 10, 13, 11, 14, 15];

/// Column-major 4×4 scan order.
pub const COL_SCAN_4X4: [i16; 16] = [0, 1, 2, 4, 3, 5, 6, 8, 7, 9, 10, 12, 13, 11, 14, 15];

/// Row-major 4×4 scan order.
pub const ROW_SCAN_4X4: [i16; 16] = [0, 4, 1, 8, 5, 12, 9, 2, 6, 13, 3, 10, 7, 14, 11, 15];

/// Default 8×8 scan order.
pub const DEFAULT_SCAN_8X8: [i16; 64] = [
    0, 1, 8, 2, 9, 16, 10, 3, 17, 24, 18, 11, 4, 25, 32, 19, 12, 26, 5, 33, 20, 27, 40, 13, 34, 6,
    41, 28, 21, 35, 42, 48, 14, 7, 36, 29, 43, 56, 49, 22, 15, 37, 50, 44, 57, 30, 23, 51, 45, 58,
    38, 31, 52, 59, 39, 46, 53, 60, 47, 54, 61, 55, 62, 63,
];

/// Column-major 8×8 scan order.
pub const COL_SCAN_8X8: [i16; 64] = [
    0, 1, 2, 8, 3, 9, 4, 10, 16, 5, 11, 17, 12, 18, 6, 24, 19, 13, 25, 7, 26, 20, 32, 14, 27, 21,
    33, 28, 34, 15, 22, 35, 40, 29, 41, 36, 23, 30, 42, 37, 48, 43, 31, 44, 49, 38, 50, 56, 45, 39,
    51, 57, 52, 46, 58, 53, 59, 47, 60, 54, 61, 55, 62, 63,
];

/// Row-major 8×8 scan order.
pub const ROW_SCAN_8X8: [i16; 64] = [
    0, 8, 16, 1, 9, 24, 2, 17, 32, 10, 25, 3, 40, 18, 11, 33, 26, 19, 4, 48, 41, 34, 12, 27, 56,
    20, 5, 42, 35, 13, 49, 28, 6, 21, 43, 36, 14, 50, 29, 57, 7, 44, 22, 37, 51, 15, 58, 30, 23,
    45, 52, 38, 59, 31, 46, 53, 39, 60, 47, 61, 54, 62, 55, 63,
];

/// Default 16×16 scan order.
pub const DEFAULT_SCAN_16X16: [i16; 256] = [
    0, 1, 16, 2, 17, 32, 3, 18, 33, 48, 4, 34, 19, 49, 20, 5, 35, 64, 50, 36, 65, 21, 6, 51, 80,
    66, 37, 22, 52, 7, 81, 67, 38, 82, 53, 23, 96, 68, 8, 83, 97, 54, 39, 69, 112, 24, 98, 84, 70,
    55, 9, 40, 85, 99, 113, 128, 25, 114, 100, 71, 86, 56, 10, 41, 115, 101, 129, 116, 72, 87, 26,
    130, 144, 102, 57, 11, 42, 117, 131, 145, 88, 103, 27, 73, 132, 118, 146, 58, 160, 12, 43, 133,
    147, 104, 89, 119, 161, 74, 148, 134, 28, 162, 59, 13, 176, 120, 149, 90, 135, 105, 163, 44,
    75, 177, 164, 29, 150, 121, 136, 178, 165, 14, 106, 60, 91, 151, 45, 179, 192, 137, 166, 122,
    76, 180, 152, 30, 61, 15, 107, 167, 181, 193, 92, 208, 46, 138, 123, 153, 194, 77, 168, 182,
    31, 195, 209, 183, 108, 139, 62, 154, 47, 196, 93, 169, 210, 197, 224, 124, 184, 211, 78, 109,
    170, 155, 63, 198, 212, 185, 225, 240, 140, 94, 199, 125, 79, 213, 226, 171, 186, 156, 214,
    200, 110, 227, 141, 95, 241, 215, 228, 201, 126, 242, 187, 172, 157, 229, 111, 216, 243, 142,
    202, 230, 127, 217, 244, 173, 188, 231, 158, 203, 143, 245, 218, 232, 189, 246, 159, 174, 233,
    247, 219, 204, 175, 190, 248, 234, 205, 220, 249, 191, 235, 221, 250, 206, 222, 251, 236, 207,
    237, 223, 252, 238, 253, 239, 254, 255,
];

/// Column-major 16×16 scan order.
pub const COL_SCAN_16X16: [i16; 256] = [
    0, 1, 2, 3, 16, 4, 17, 5, 18, 6, 19, 32, 20, 7, 33, 21, 34, 8, 35, 22, 48, 36, 9, 49, 23, 50,
    37, 10, 38, 51, 24, 64, 52, 11, 65, 39, 25, 53, 66, 54, 40, 67, 12, 80, 26, 68, 55, 81, 41, 69,
    13, 27, 82, 56, 70, 83, 42, 14, 84, 96, 71, 28, 57, 85, 97, 15, 72, 98, 43, 86, 58, 99, 29, 87,
    100, 112, 73, 44, 101, 59, 30, 113, 88, 114, 74, 128, 102, 45, 31, 115, 60, 103, 89, 116, 75,
    129, 117, 46, 104, 90, 61, 130, 118, 131, 132, 105, 76, 47, 119, 144, 91, 62, 133, 106, 145,
    120, 146, 134, 77, 147, 121, 92, 135, 148, 63, 107, 136, 122, 93, 149, 160, 78, 150, 137, 108,
    161, 162, 151, 123, 79, 138, 163, 152, 94, 164, 109, 165, 153, 124, 139, 176, 166, 95, 177,
    167, 110, 154, 178, 125, 179, 140, 168, 155, 111, 180, 192, 181, 169, 141, 126, 182, 193, 194,
    156, 183, 170, 195, 127, 142, 196, 184, 208, 197, 157, 171, 143, 185, 198, 209, 199, 210, 172,
    158, 186, 211, 224, 212, 200, 240, 159, 213, 225, 187, 201, 173, 226, 214, 215, 227, 202, 228,
    188, 241, 216, 174, 229, 242, 203, 243, 217, 230, 175, 189, 244, 231, 204, 218, 232, 245, 219,
    246, 190, 233, 205, 191, 247, 234, 248, 220, 206, 249, 235, 221, 207, 250, 236, 222, 251, 223,
    237, 238, 252, 239, 253, 254, 255,
];

/// Row-major 16×16 scan order.
pub const ROW_SCAN_16X16: [i16; 256] = [
    0, 16, 32, 1, 48, 17, 64, 33, 2, 80, 18, 49, 96, 34, 3, 65, 19, 112, 50, 81, 35, 4, 128, 66,
    20, 97, 51, 82, 5, 144, 36, 67, 113, 98, 21, 52, 160, 83, 129, 37, 68, 6, 114, 176, 99, 53, 22,
    84, 145, 38, 69, 130, 7, 115, 192, 100, 54, 23, 85, 161, 146, 131, 39, 70, 208, 116, 8, 101,
    177, 55, 86, 24, 162, 147, 132, 71, 224, 117, 40, 102, 9, 148, 56, 87, 193, 163, 240, 133, 178,
    25, 118, 72, 41, 103, 164, 10, 149, 88, 134, 209, 179, 57, 119, 194, 26, 73, 165, 150, 104, 42,
    135, 11, 180, 120, 89, 225, 195, 58, 27, 210, 151, 181, 166, 74, 43, 105, 12, 136, 90, 59, 241,
    121, 28, 196, 167, 211, 152, 44, 182, 137, 75, 13, 226, 106, 122, 60, 197, 91, 168, 29, 183,
    153, 14, 76, 212, 138, 45, 107, 15, 198, 92, 227, 169, 30, 123, 154, 61, 242, 184, 213, 139,
    46, 77, 31, 108, 170, 199, 185, 124, 228, 93, 155, 214, 62, 140, 243, 78, 47, 200, 109, 186,
    171, 201, 94, 63, 215, 229, 156, 79, 125, 141, 110, 216, 187, 172, 244, 202, 230, 217, 95, 157,
    126, 245, 111, 142, 231, 188, 127, 158, 218, 173, 232, 246, 233, 203, 143, 247, 174, 189, 159,
    219, 204, 248, 234, 249, 175, 190, 220, 205, 250, 235, 191, 221, 251, 236, 206, 252, 222, 207,
    237, 223, 253, 238, 254, 239, 255,
];

/// Default 32×32 scan order (1024 entries).
/// Translated from FFmpeg vp9data.c `ff_vp9_default_scan_32x32`.
pub const DEFAULT_SCAN_32X32: [i16; 1024] = [
    0, 1, 32, 2, 33, 64, 3, 34, 65, 4, 96, 35, 66, 5, 36, 97, 67, 128, 98, 68, 37, 6, 129, 99, 7,
    160, 69, 38, 130, 100, 161, 131, 39, 70, 8, 101, 162, 132, 192, 71, 40, 9, 102, 163, 133, 193,
    72, 224, 103, 41, 164, 10, 194, 134, 165, 73, 104, 135, 225, 42, 195, 11, 256, 166, 226, 196,
    74, 105, 136, 43, 12, 167, 197, 227, 257, 75, 106, 137, 228, 44, 198, 168, 258, 288, 13, 229,
    76, 107, 199, 138, 259, 169, 289, 45, 230, 260, 200, 108, 14, 170, 139, 320, 290, 77, 231, 261,
    46, 201, 140, 291, 109, 232, 321, 262, 171, 78, 292, 15, 322, 202, 263, 352, 172, 293, 233,
    141, 323, 110, 47, 203, 264, 234, 294, 353, 324, 16, 79, 204, 265, 295, 325, 173, 354, 142,
    235, 384, 48, 296, 111, 266, 355, 326, 80, 17, 205, 236, 174, 356, 385, 327, 143, 297, 267,
    357, 386, 112, 49, 328, 298, 206, 416, 237, 358, 387, 81, 175, 18, 329, 359, 388, 299, 330,
    389, 113, 417, 238, 360, 50, 207, 418, 390, 331, 19, 448, 361, 82, 419, 391, 239, 51, 362, 420,
    114, 449, 480, 421, 83, 363, 450, 422, 512, 451, 423, 115, 452, 481, 453, 482, 454, 544, 483,
    455, 513, 484, 514, 485, 515, 486, 545, 576, 487, 546, 547, 608, 577, 578, 579, 609, 610, 611,
    20, 144, 268, 392, 516, 640, 21, 52, 145, 176, 269, 300, 393, 424, 517, 548, 641, 672, 22, 53,
    84, 146, 177, 208, 270, 301, 332, 394, 425, 456, 518, 549, 580, 642, 673, 704, 23, 54, 85, 116,
    147, 178, 209, 240, 271, 302, 333, 364, 395, 426, 457, 488, 519, 550, 581, 612, 643, 674, 705,
    736, 55, 86, 117, 179, 210, 241, 303, 334, 365, 427, 458, 489, 551, 582, 613, 675, 706, 737,
    87, 118, 211, 242, 335, 366, 459, 490, 583, 614, 707, 738, 119, 243, 367, 491, 615, 739, 24,
    148, 272, 396, 520, 644, 768, 25, 56, 149, 180, 273, 304, 397, 428, 521, 552, 645, 676, 769,
    800, 26, 57, 88, 150, 181, 212, 274, 305, 336, 398, 429, 460, 522, 553, 584, 646, 677, 708,
    770, 801, 832, 27, 58, 89, 120, 151, 182, 213, 244, 275, 306, 337, 368, 399, 430, 461, 492,
    523, 554, 585, 616, 647, 678, 709, 740, 771, 802, 833, 864, 59, 90, 121, 183, 214, 245, 307,
    338, 369, 431, 462, 493, 555, 586, 617, 679, 710, 741, 803, 834, 865, 91, 122, 215, 246, 339,
    370, 463, 494, 587, 618, 711, 742, 835, 866, 123, 247, 371, 495, 619, 743, 867, 28, 152, 276,
    400, 524, 648, 772, 896, 29, 60, 153, 184, 277, 308, 401, 432, 525, 556, 649, 680, 773, 804,
    897, 928, 30, 61, 92, 154, 185, 216, 278, 309, 340, 402, 433, 464, 526, 557, 588, 650, 681,
    712, 774, 805, 836, 898, 929, 960, 31, 62, 93, 124, 155, 186, 217, 248, 279, 310, 341, 372,
    403, 434, 465, 496, 527, 558, 589, 620, 651, 682, 713, 744, 775, 806, 837, 868, 899, 930, 961,
    992, 63, 94, 125, 187, 218, 249, 311, 342, 373, 435, 466, 497, 559, 590, 621, 683, 714, 745,
    807, 838, 869, 931, 962, 993, 95, 126, 219, 250, 343, 374, 467, 498, 591, 622, 715, 746, 839,
    870, 963, 994, 127, 251, 375, 499, 623, 747, 871, 995, 156, 280, 404, 528, 652, 776, 900, 157,
    188, 281, 312, 405, 436, 529, 560, 653, 684, 777, 808, 901, 932, 158, 189, 220, 282, 313, 344,
    406, 437, 468, 530, 561, 592, 654, 685, 716, 778, 809, 840, 902, 933, 964, 159, 190, 221, 252,
    283, 314, 345, 376, 407, 438, 469, 500, 531, 562, 593, 624, 655, 686, 717, 748, 779, 810, 841,
    872, 903, 934, 965, 996, 191, 222, 253, 315, 346, 377, 439, 470, 501, 563, 594, 625, 687, 718,
    749, 811, 842, 873, 935, 966, 997, 223, 254, 347, 378, 471, 502, 595, 626, 719, 750, 843, 874,
    967, 998, 255, 379, 503, 627, 751, 875, 999, 284, 408, 532, 656, 780, 904, 285, 316, 409, 440,
    533, 564, 657, 688, 781, 812, 905, 936, 286, 317, 348, 410, 441, 472, 534, 565, 596, 658, 689,
    720, 782, 813, 844, 906, 937, 968, 287, 318, 349, 380, 411, 442, 473, 504, 535, 566, 597, 628,
    659, 690, 721, 752, 783, 814, 845, 876, 907, 938, 969, 1000, 319, 350, 381, 443, 474, 505, 567,
    598, 629, 691, 722, 753, 815, 846, 877, 939, 970, 1001, 351, 382, 475, 506, 599, 630, 723, 754,
    847, 878, 971, 1002, 383, 507, 631, 755, 879, 1003, 412, 536, 660, 784, 908, 413, 444, 537,
    568, 661, 692, 785, 816, 909, 940, 414, 445, 476, 538, 569, 600, 662, 693, 724, 786, 817, 848,
    910, 941, 972, 415, 446, 477, 508, 539, 570, 601, 632, 663, 694, 725, 756, 787, 818, 849, 880,
    911, 942, 973, 1004, 447, 478, 509, 571, 602, 633, 695, 726, 757, 819, 850, 881, 943, 974,
    1005, 479, 510, 603, 634, 727, 758, 851, 882, 975, 1006, 511, 635, 759, 883, 1007, 540, 664,
    788, 912, 541, 572, 665, 696, 789, 820, 913, 944, 542, 573, 604, 666, 697, 728, 790, 821, 852,
    914, 945, 976, 543, 574, 605, 636, 667, 698, 729, 760, 791, 822, 853, 884, 915, 946, 977, 1008,
    575, 606, 637, 699, 730, 761, 823, 854, 885, 947, 978, 1009, 607, 638, 731, 762, 855, 886, 979,
    1010, 639, 763, 887, 1011, 668, 792, 916, 669, 700, 793, 824, 917, 948, 670, 701, 732, 794,
    825, 856, 918, 949, 980, 671, 702, 733, 764, 795, 826, 857, 888, 919, 950, 981, 1012, 703, 734,
    765, 827, 858, 889, 951, 982, 1013, 735, 766, 859, 890, 983, 1014, 767, 891, 1015, 796, 920,
    797, 828, 921, 952, 798, 829, 860, 922, 953, 984, 799, 830, 861, 892, 923, 954, 985, 1016, 831,
    862, 893, 955, 986, 1017, 863, 894, 987, 1018, 895, 1019, 924, 925, 956, 926, 957, 988, 927,
    958, 989, 1020, 959, 990, 1021, 991, 1022, 1023,
];

// ---------------------------------------------------------------------------
// Pareto8 table  (ff_vp9_model_pareto8)
// ---------------------------------------------------------------------------

/// Pareto distribution for 8-symbol alphabets, used in probability
/// interpolation. 256 rows × 8 columns.
pub const PARETO8: [[u8; 8]; 256] = [
...
// ---------------------------------------------------------------------------
// Default probability context  (ff_vp9_default_probs)
// ---------------------------------------------------------------------------

/// Default frame-level probabilities (non-keyframe).
/// Matches `ff_vp9_default_probs` in vp9data.c.
pub mod default_probs {
    pub const Y_MODE: [[u8; 9]; 4] = [
    ...
    pub const FILTER: [[u8; 2]; 4] = [[235, 162], [36, 255], [34, 3], [149, 144]];

    pub const MV_MODE: [[u8; 3]; 7] = [
        [2, 173, 34],
        [7, 145, 85],
        [7, 166, 63],
        [7, 94, 66],
    ...
    pub const INTRA: [u8; 4] = [9, 102, 187, 225];

    pub const COMP: [u8; 5] = [239, 183, 119, 96, 41];

    pub const SINGLE_REF: [[u8; 2]; 5] = [[33, 16], [77, 74], [142, 142], [172, 170], [238, 247]];

    pub const COMP_REF: [u8; 5] = [50, 126, 123, 221, 226];

    pub const TX32P: [[u8; 3]; 2] = [[3, 136, 37], [5, 52, 13]];

    pub const TX16P: [[u8; 2]; 2] = [[20, 152], [15, 101]];

    pub const TX8P: [u8; 2] = [100, 66];

    pub const SKIP: [u8; 3] = [192, 128, 64];

    pub const MV_JOINT: [u8; 3] = [32, 64, 96];

    /// MV component probabilities: [sign, classes[10], class0, bits[10],
    ///                               class0_fp[2][3], fp[3], class0_hp, hp]
    /// Index 0 = vertical, index 1 = horizontal.
    pub const MV_COMP_SIGN: [u8; 2] = [128, 128];
    pub const MV_COMP_CLASSES: [[u8; 10]; 2] = [
        [224, 144, 192, 168, 192, 176, 192, 198, 198, 245],
        [216, 128, 176, 160, 176, 176, 192, 198, 198, 208],
    ];
    pub const MV_COMP_CLASS0: [u8; 2] = [216, 208];
    pub const MV_COMP_BITS: [[u8; 10]; 2] = [
        [136, 140, 148, 160, 176, 192, 224, 234, 234, 240],
        [136, 140, 148, 160, 176, 192, 224, 234, 234, 240],
    ];
    pub const MV_COMP_CLASS0_FP: [[[u8; 3]; 2]; 2] = [
        [[128, 128, 64], [96, 112, 64]],
        [[128, 128, 64], [96, 112, 64]],
    ];
    pub const MV_COMP_FP: [[u8; 3]; 2] = [[64, 96, 64], [64, 96, 64]];
    pub const MV_COMP_CLASS0_HP: [u8; 2] = [160, 160];
    pub const MV_COMP_HP: [u8; 2] = [128, 128];

    pub const PARTITION: [[[[u8; 3]; 4]; 4]; 4] = [
        /* 64x64 → 32x32 */
        [
...
// ---------------------------------------------------------------------------
// Default coefficient probabilities  (ff_vp9_default_coef_probs)
// ---------------------------------------------------------------------------

/// Per-band, per-context coefficient probabilities: [6 contexts][3 probs].
pub type CoefBandProbs = [[u8; 3]; 6];
/// Per-transform-coefficient-band set: [6 bands][6 contexts][3 probs].
pub type CoefPlaneProbs = [CoefBandProbs; 6];
/// Intra + Inter planes: [block_type][intra_flag][6 bands][6 ctx][3].
pub type CoefBlockProbs = [[CoefPlaneProbs; 2]; 2];
/// Full coefficient probability table: [4 tx_sizes][2 block_types][2][6][6][3].
pub type CoefProbs = [CoefBlockProbs; 4];

/// Default coefficient probabilities [tx_size][block_type][intra][band][ctx][3].
/// Dimensions: [4][2][2][6][6][3]
/// Translated from FFmpeg vp9data.c `ff_vp9_default_coef_probs`.
...
// ---------------------------------------------------------------------------
// Scan neighbour tables  (ff_vp9_*_scan_*_nb from vp9data.c)
// ---------------------------------------------------------------------------
// Each entry is [left_nb, above_nb] giving the two neighbours used to
// compute the coefficient context for one scan position.

/// Default 4×4 scan neighbours.
pub const DEFAULT_SCAN_4X4_NB: [[i16; 2]; 16] = [
    [0, 0],
    [0, 0],
    [4, 1],
    [1, 1],
...
/// Column-major 4×4 scan neighbours.
pub const COL_SCAN_4X4_NB: [[i16; 2]; 16] = [
    [0, 0],
    [1, 1],
    [0, 0],
    [2, 2],
...
/// Row-major 4×4 scan neighbours.
pub const ROW_SCAN_4X4_NB: [[i16; 2]; 16] = [
    [0, 0],
    [0, 0],
    [4, 4],
    [1, 1],
...
/// Default 8×8 scan neighbours.
pub const DEFAULT_SCAN_8X8_NB: [[i16; 2]; 64] = [
    [0, 0],
    [0, 0],
    [1, 1],
    [8, 1],
...
/// Column-major 8×8 scan neighbours.
pub const COL_SCAN_8X8_NB: [[i16; 2]; 64] = [
    [0, 0],
    [1, 1],
    [0, 0],
    [2, 2],
...
/// Row-major 8×8 scan neighbours.
pub const ROW_SCAN_8X8_NB: [[i16; 2]; 64] = [
    [0, 0],
    [8, 8],
    [0, 0],
    [1, 1],
...
/// Default 16×16 scan neighbours.
pub const DEFAULT_SCAN_16X16_NB: [[i16; 2]; 256] = [
    [0, 0],
    [0, 0],
    [1, 1],
    [16, 1],
...
/// Column-major 16×16 scan neighbours.
pub const COL_SCAN_16X16_NB: [[i16; 2]; 256] = [
    [0, 0],
    [1, 1],
    [2, 2],
    [0, 0],
...
/// Row-major 16×16 scan neighbours.
pub const ROW_SCAN_16X16_NB: [[i16; 2]; 256] = [
    [0, 0],
    [16, 16],
    [0, 0],
    [32, 32],
...
/// Default 32×32 scan neighbours (only default scan exists for 32×32).
#[rustfmt::skip]
pub const DEFAULT_SCAN_32X32_NB: [[i16; 2]; 1024] = [
    [0, 0], [0, 0], [1, 1], [32, 1],
...
// ---------------------------------------------------------------------------
// Inter-prediction tables
// ---------------------------------------------------------------------------

/// MV reference block offsets: [block_size][candidate_idx] -> (col_offset, row_offset).
///
/// Used in find_ref_mvs() to locate neighbouring MV candidates.
/// Verbatim from vp9mvs.c `mv_ref_blk_off`.
#[rustfmt::skip]
...
/// Inter mode context lookup: [above_mode_ctx][left_mode_ctx] -> context (0-6).
///
/// Verbatim from vp9block.c `inter_mode_ctx_lut`.
...
/// Column offset for sub-8x8 inter mode context lookup.
///
/// Indexed by block size (BS_64x64=0 through BS_8x8=9).
/// From vp9block.c decode_mode().
pub const INTER_MODE_CTX_OFF: [u8; 10] = [3, 0, 0, 1, 0, 0, 0, 0, 0, 0];

/// Maps block size (BS_64x64..BS_8x8) to y_mode probability group index (0-3).
///
/// Only valid for block sizes 0-9. Sub-8x8 blocks use a different code path.
/// From vp9block.c decode_mode().
pub const SIZE_GROUP: [u8; 10] = [3, 3, 3, 3, 2, 2, 2, 1, 1, 1];

/// 8-tap subpel interpolation filter coefficients.
///
/// `[filter_type][subpel_position][tap]`
/// - filter_type: 0=Smooth, 1=Regular, 2=Sharp
/// - subpel_position: 0-15 (0 = integer pel)
/// - tap: 0-7
///
/// Verbatim from vp9dsp.c `ff_vp9_subpel_filters`.
#[rustfmt::skip]
pub const VP9_SUBPEL_FILTERS: [[[i16; 8]; 16]; 4] = [
